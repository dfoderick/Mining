...
yourminerapiscript = "minerapi3.py"
...
while True:
	try:
		...
		apiresponse = subprocess.run(["python",yourminerapiscript, apistatscommand, yourmineripaddress, yourminerport], stdout=subprocess.PIPE, shell=True, check=True).stdout
		#fudge the json return results, apparently a bug in cgminer stats result
		statsstart = apiresponse.find(b'}{"STATS"')+1
		statsend = apiresponse.find(b'}],"id":1}')+1
